...
class Validators {
  ...
  static final RegExp _currency = RegExp(
    r'^\d+(\.\d{1,2})?$',
  ); // จำนวนเงิน ทศนิยมไม่เกิน 2 หลัก
  static final RegExp _baht = RegExp(
    r'^฿\d+(\.\d{1,2})?$',
  ); // จำนวนเงินสกุลบาทไทย (฿)
  static final RegExp _dollar = RegExp(
    r'^\$\d+(\.\d{1,2})?$',
  ); // จำนวนเงินสกุลดอลลาร์ ($)
  ...
  static final RegExp _hasWhitespace = RegExp(
    r'\s',
  ); // ตรวจจับช่องว่าง (whitespace)
  static final RegExp _repeatChar = RegExp(
    r'(.)\1{2,}',
  ); // ตัวอักษรซ้ำติดกันตั้งแต่ 3 ตัวขึ้นไป
  ...
  static final RegExp _thaiPhone = RegExp(
    r'^(0[689]{1})\d{8}$',
  ); // เบอร์โทรศัพท์ไทย 10 หลัก เริ่มด้วย 06/08/09
  static final RegExp _thaiZip = RegExp(
    r'^[1-9][0-9]{4}$',
  ); // รหัสไปรษณีย์ไทย 5 หลัก (ขึ้นต้น 1-9)
  static final RegExp _visa = RegExp(
    r'^4[0-9]{12}(?:[0-9]{3})?$',
  ); // เลขบัตร Visa (ขึ้นต้นด้วย 4)
  static final RegExp _mastercard = RegExp(
    r'^5[1-5][0-9]{14}$',
  ); // เลขบัตร MasterCard (ขึ้นต้นด้วย 51-55)
  static final RegExp _alpha = RegExp(
    r'^[A-Za-z]+$',
  ); // ตัวอักษรภาษาอังกฤษเท่านั้น
  static final RegExp _alnum = RegExp(
    r'^[A-Za-z0-9]+$',
  ); // ตัวอักษรและตัวเลขเท่านั้น
  static final RegExp _ascii = RegExp(
    r'^[\x00-\x7F]+$',
  ); // อักขระ ASCII เท่านั้น
  static final RegExp _hexColor = RegExp(
    r'^#?([0-9A-Fa-f]{6}|[0-9A-Fa-f]{3})$',
  ); // สี Hex 3 หรือ 6 หลัก (มี/ไม่มี #)
  static final RegExp _url = RegExp(
    r"^(https?:\/\/)?([\w\-]+\.)+[\w\-]+(\/[\w\-\._\?\,\'\/\\\+&%\$#=~]*)?",
  ); // รูปแบบ URL (http/https)
  static final RegExp _email = RegExp(
    r'^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$',
  ); // อีเมลพื้นฐาน name@domain.tld
  static final RegExp _phone = RegExp(
    r'^(\+\d{1,3}[- ]?)?\d{9,15}$',
  ); // เบอร์โทรศัพท์สากล 9-15 หลัก รองรับ +รหัสประเทศ
  static final RegExp _username = RegExp(
    r'^[A-Za-z0-9_]{3,20}$',
  ); // ชื่อผู้ใช้ A-Z a-z 0-9 และ _ ความยาว 3-20 ตัว
  static final RegExp _usernameLettersOnly = RegExp(
    r'^[A-Za-z]{3,20}$',
  ); // ชื่อผู้ใช้: ตัวอักษรเท่านั้น
  static final RegExp _usernameLowercaseOnly = RegExp(
    r'^[a-z]{3,20}$',
  ); // ชื่อผู้ใช้: ตัวอักษรพิมพ์เล็กเท่านั้น
  static final RegExp _usernameAlnumOnly = RegExp(
    r'^[A-Za-z0-9]{3,20}$',
  ); // ชื่อผู้ใช้: ตัวอักษรและตัวเลข (ไม่มีอักขระพิเศษ)
  ...
  static final RegExp _pwdNumber = RegExp(
    r'^[0-9]+$',
  ); // รหัสผ่านเป็นตัวเลขล้วน
  static final RegExp _pwdNumberText = RegExp(
    r'^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]+$',
  ); // รหัสผ่านมีตัวอักษรและตัวเลข
  static final RegExp _pwdNumberOrText = RegExp(
    r'^[A-Za-z0-9]+$',
  ...
  static final RegExp _pwdLettersOnlyWithUpper = RegExp(
    r'^(?=.*[A-Z])[A-Za-z]+$',
  ); // รหัสผ่าน: ตัวอักษรเท่านั้น และต้องมีตัวพิมพ์ใหญ่ ≥1
  static final RegExp _thaiOnly = RegExp(
    r'^[ก-๙\s]+$',
  ); // ข้อความภาษาไทยและช่องว่างเท่านั้น
  static final RegExp _engOnly = RegExp(
    r'^[A-Za-z\s]+$',
  ); // ข้อความภาษาอังกฤษและช่องว่างเท่านั้น
  ...
  static final RegExp _nationalId13 = RegExp(
    r'^\d{13}$',
  ); // ตัวเลข 13 หลักสำหรับบัตรประชาชนไทย
  ...
  static Validator isUUIDv4([String? message]) {
    ...
    return (value) => (value is String && !_uuidV4.hasMatch(value))
        ? (message ?? Messages.t('uuid'))
        : null;
  }
  ...
  static Validator isIPv4([String? message]) {
    ...
    return (value) => (value is String && !_ipv4.hasMatch(value))
        ? (message ?? Messages.t('ip'))
        : null;
  }
  ...
  static Validator isIPv6([String? message]) {
    ...
    return (value) => (value is String && !_ipv6.hasMatch(value))
        ? (message ?? Messages.t('ip'))
        : null;
  }
  ...
  static Validator isCurrency([String? message]) {
    ...
    return (value) => (value is String && !_currency.hasMatch(value))
        ? (message ?? Messages.t('currency'))
        : null;
  }
  ...
  static Validator isBaht([String? message]) {
    ...
    return (value) => (value is String && !_baht.hasMatch(value))
        ? (message ?? Messages.t('currency'))
        : null;
  }
  ...
  static Validator isDollar([String? message]) {
    ...
    return (value) => (value is String && !_dollar.hasMatch(value))
        ? (message ?? Messages.t('currency'))
        : null;
  }
  ...
  static Validator isEAN13([String? message]) {
    ...
    return (value) => (value is String && !_ean13.hasMatch(value))
        ? (message ?? Messages.t('barcode'))
        : null;
  }
  ...
  static Validator isEAN8([String? message]) {
    ...
    return (value) => (value is String && !_ean8.hasMatch(value))
        ? (message ?? Messages.t('barcode'))
        : null;
  }
  ...
  static Validator isEAN8Checksum([String? message]) {
    return (value) {
      ...
      return check == last
          ? null
          : (message ??
                Messages.t('invalid_ean8', fallback: 'Invalid EAN-8 checksum'));
    };
  }
  ...
  static Validator isEAN13Checksum([String? message]) {
    return (value) {
      if (value is! String || !_ean13.hasMatch(value)) {
        return message ??
            Messages.t('invalid_ean13', fallback: 'Invalid EAN-13');
      }
      int sum = 0;
      for (int i = 0; i < 12; i++) {
        final d = value.codeUnitAt(i) - 48; // '0' => 48
      ...
      return check == last
          ...
          : (message ??
                Messages.t(
                  'invalid_ean13',
                  fallback: 'Invalid EAN-13 checksum',
                ));
    };
  }
  ...
  static Validator noWhitespace([String? message]) {
    // ห้ามมีช่องว่างในค่า
    return (value) => (value is String && _hasWhitespace.hasMatch(value))
        ? (message ??
              Messages.t('no_whitespace', fallback: 'No whitespace allowed'))
        : null;
  }
  ...
  static Validator noRepeatChar([String? message]) {
    ...
    return (value) => (value is String && _repeatChar.hasMatch(value))
        ? (message ??
              Messages.t('no_repeat_char', fallback: 'No repeated characters'))
        : null;
  }
  ...
  static Validator noSequentialChar([String? message]) {
    ...
    return (value) =>
        (value is String && _sequential.hasMatch(value.toLowerCase()))
        ? (message ??
              Messages.t(
                'no_sequential_char',
                fallback: 'No sequential characters',
              ))
        : null;
  }
  ...
  static Validator isPalindrome([String? message]) {
    ...
    return (value) {
      ...
      return s == s.split('').reversed.join('')
          ? null
          : (message ??
                Messages.t('not_palindrome', fallback: 'Not a palindrome'));
    };
  }
  ...
  static Validator isPrime([String? message]) {
    ...
    return (value) {
      ...
      if (n == null || n < 2)
        return (message ??
            Messages.t('not_prime', fallback: 'Not a prime number'));
  ...
  static Validator isMultipleOf(int n, [String? message]) {
    ...
    return (value) {
      ...
      return (v != null && v % n == 0)
          ? null
          : (message ??
                Messages.t('not_multiple', fallback: 'Not a multiple of $n'));
    };
  }
  ...
  static Validator isThaiPhone([String? message]) {
    ...
    return (value) => (value is String && !_thaiPhone.hasMatch(value))
        ? (message ?? Messages.t('phone'))
        : null;
  }
  ...
  static Validator isThaiZipCode([String? message]) {
    ...
    return (value) => (value is String && !_thaiZip.hasMatch(value))
        ? (message ?? Messages.t('invalid_zip_code'))
        : null;
  }
  ...
  static Validator isVisaCard([String? message]) {
    ...
    return (value) => (value is String && !_visa.hasMatch(value))
        ? (message ?? Messages.t('invalid_visa'))
        : null;
  }
  ...
  static Validator isMasterCard([String? message]) {
    ...
    return (value) => (value is String && !_mastercard.hasMatch(value))
        ? (message ?? Messages.t('invalid_mastercard'))
        : null;
  }
  ...
  static Validator isTrue([String? message]) {
    ...
    return (value) => value == true
        ? null
        : (message ??
              Messages.t('value_must_be_true', fallback: 'Value must be true'));
  }
  ...
  static Validator isEnumValue(List enumValues, [String? message]) {
    ...
    return (value) => set.contains(value)
        ? null
        : (message ?? Messages.t('not_in_enum', fallback: 'Value not in enum'));
  }
  ...
  static Validator notMatch(RegExp pattern, [String? message]) {
    ...
    return (value) => (value is String && pattern.hasMatch(value))
        ? (message ?? Messages.t('invalid_value', fallback: 'Invalid value'))
        : null;
  }
  ...
  static Validator isAlpha([String? message]) {
    ...
    return (value) => (value is String && !_alpha.hasMatch(value))
        ? (message ?? Messages.t('only_letters'))
        : null;
  }
  ...
  static Validator isAlphanumeric([String? message]) {
    ...
    return (value) => (value is String && !_alnum.hasMatch(value))
        ? (message ?? Messages.t('only_letters_numbers'))
        : null;
  }
  ...
  static Validator isAscii([String? message]) {
    ...
    return (value) => (value is String && !_ascii.hasMatch(value))
        ? (message ?? Messages.t('only_ascii'))
        : null;
  }
  ...
  static Validator isHexColor([String? message]) {
    ...
    return (value) => (value is String && !_hexColor.hasMatch(value))
        ? (message ?? Messages.t('invalid_hex_color'))
        : null;
  }
  ...
  static Validator isFileExtension(List<String> exts, [String? message]) {
    ...
    return (value) => (value is String && !exts.any((e) => value.endsWith(e)))
        ? (message ?? Messages.t('invalid_file_extension'))
        : null;
  }
  ...
  static Validator isUrl([String? message]) {
    // ตรวจสอบรูปแบบ URL
    return (value) => (value is String && !_url.hasMatch(value))
        ? (message ?? Messages.t('invalid_url'))
        : null;
  }
  ...
  static Validator isEmailDomain(String domain, [String? message]) {
    ...
    return (value) =>
        (value is String && value.contains('@') && !value.endsWith('@$domain'))
        ? (message ??
              Messages.t(
                'email_must_end_with',
                fallback: 'Email must end with @$domain',
              ))
        : null;
  }

  /// Email must belong to one of the allowed domains (case-insensitive)
  static Validator isEmailInDomains(
    ...
    return (value) {
      ...
      return set.contains(domain)
          ...
          : (message ??
                Messages.t(
                  'email_must_end_with',
                  fallback: 'Email must be in allowed domains',
                ));
    };
  }
  ...
  static Validator isEmpty([String? message]) {
    ...
    return (value) {
      if (value == null) return (message ?? Messages.t('value_empty'));
  ...
  static Validator isNotEmpty([String? message]) {
    ...
    return (value) {
      if (value == null) return (message ?? Messages.t('value_not_empty'));
      if (value is String && value.trim().isEmpty)
  ...
  static Validator isNull([String? message]) {
    ...
    return (value) =>
        value == null ? (message ?? Messages.t('value_null')) : null;
  }
  ...
  static Validator isNotNull([String? message]) {
    // ต้องไม่เป็นค่า null
    return (value) =>
        value != null ? null : (message ?? Messages.t('value_not_null'));
  }

  static Validator onlyThaiText([String? message]) {
    // อนุญาตเฉพาะภาษาไทยและช่องว่าง
    return (value) => (value is String && !_thaiOnly.hasMatch(value))
        ? (message ?? Messages.t('only_thai'))
        : null;
  }

  static Validator onlyEnglishText([String? message]) {
    // อนุญาตเฉพาะภาษาอังกฤษและช่องว่าง
    return (value) => (value is String && !_engOnly.hasMatch(value))
        ? (message ?? Messages.t('only_english'))
        : null;
  }

  static Validator isLowerCase([String? message]) {
    // ต้องเป็นตัวพิมพ์เล็กทั้งหมด
    return (value) => (value is String && value != value.toLowerCase())
        ? (message ?? Messages.t('only_lowercase'))
        : null;
  }

  static Validator isUpperCase([String? message]) {
    // ต้องเป็นตัวพิมพ์ใหญ่ทั้งหมด
    return (value) => (value is String && value != value.toUpperCase())
        ? (message ?? Messages.t('only_uppercase'))
        : null;
  }
  ...
  static Validator pattern(RegExp pattern, [String? message]) {
    ...
    return (value) => (value is String && !pattern.hasMatch(value))
        ? (message ?? Messages.t('invalid_format'))
        : null;
  }

  static Validator minLength(int min, [String? message]) {
    // ความยาวอย่างน้อย min ตัวอักษร
    return (value) => (value is String && value.length < min)
        ? (message ?? Messages.t('minLength'))
        : null;
  }

  static Validator maxLength(int max, [String? message]) {
    // ความยาวไม่เกิน max ตัวอักษร
    return (value) => (value is String && value.length > max)
        ? (message ??
              Messages.t('maxLength', fallback: 'Maximum $max characters'))
        : null;
  }
  ...
  static Validator isInt([String? message]) {
    ...
    return (value) =>
        (value is int || (value is String && int.tryParse(value) != null))
        ? null
        : (message ?? Messages.t('not_integer'));
  }

  static Validator isDouble([String? message]) {
    // ต้องเป็นจำนวนทศนิยม (double)
    return (value) =>
        (value is double || (value is String && double.tryParse(value) != null))
        ? null
        : (message ?? Messages.t('not_double'));
  }
  ...
  static Validator isBool([String? message]) {
    ...
    return (value) =>
        ...
        ? null
        : (message ?? Messages.t('not_boolean'));
  }

  static Validator isPositive([String? message]) {
    // ต้องเป็นจำนวนบวก (> 0)
    return (value) => (value is num && value > 0)
        ? null
        : (message ?? Messages.t('not_positive'));
  }
  ...
  static Validator isNegative([String? message]) {
    ...
    return (value) => (value is num && value < 0)
        ? null
        : (message ?? Messages.t('not_negative'));
  }

  static Validator isEven([String? message]) {
    // ต้องเป็นเลขคู่
    return (value) => (value is int && value % 2 == 0)
        ? null
        : (message ?? Messages.t('not_even'));
  }

  static Validator isOdd([String? message]) {
    // ต้องเป็นเลขคี่
    return (value) => (value is int && value % 2 == 1)
        ? null
        : (message ?? Messages.t('not_odd'));
  }
  ...
  static Validator isInList(List values, [String? message]) {
    ...
    return (value) =>
        set.contains(value) ? null : (message ?? Messages.t('not_in_list'));
  }
  ...
  static Validator isNotInList(List values, [String? message]) {
    ...
    final set = values is Set ? values : values.toSet();
    return (value) =>
        !set.contains(value) ? null : (message ?? Messages.t('not_allowed'));
  }

  static Validator notEmptyList([String? message]) {
    // รายการต้องไม่ว่างเปล่า
    return (value) => (value is List && value.isEmpty)
        ? (message ?? Messages.t('list_empty'))
        : null;
  }
  ...
  static Validator date([String? message]) {
    ...
    return (value) {
      ...
      return (value is String && _date.hasMatch(value))
          ? null
          : (message ?? Messages.t('date'));
    };
  }
  ...
  static Validator datetime([String? message]) {
    ...
    return (value) {
      if (value is DateTime) return null;
      return (value is String && _datetime.hasMatch(value))
          ? null
          : (message ?? Messages.t('datetime'));
    };
  }
  ...
  static Validator isFutureDate([String? message]) {
    ...
    return (value) {
      ...
      return (date != null && date.isAfter(now))
          ? null
          : (message ?? Messages.t('future_date'));
    };
  }
  ...
  static Validator otp([String? message]) {
    ...
    return (value) => (value is String && _otp.hasMatch(value))
        ? null
        : (message ?? Messages.t('invalid_otp'));
  }
  ...
  static Validator iban([String? message]) {
    ...
    return (value) {
      ...
      return result == 1
          ? null
          : (message ?? Messages.t('invalid_iban', fallback: 'Invalid IBAN'));
    };
  }
  ...
  static Validator isISBN10([String? message]) {
    return (value) {
      ...
      if (s.length != 10)
        return message ??
            Messages.t('invalid_isbn10', fallback: 'Invalid ISBN-10');
      int sum = 0;
      for (int i = 0; i < 9; i++) {
        final c = s.codeUnitAt(i);
        if (c < 48 || c > 57)
      ...
      return (sum % 11 == 0)
          ...
          : (message ??
                Messages.t(
                  'invalid_isbn10',
                  fallback: 'Invalid ISBN-10 checksum',
                ));
    };
  }
  ...
  static Validator isISBN13([String? message]) {
    return (value) {
      ...
      if (!_ean13.hasMatch(s))
        return message ??
            Messages.t('invalid_isbn13', fallback: 'Invalid ISBN-13');
      int sum = 0;
      for (int i = 0; i < 12; i++) {
      ...
      return check == last
          ...
          : (message ??
                Messages.t(
                  'invalid_isbn13',
                  fallback: 'Invalid ISBN-13 checksum',
                ));
    };
  }
  ...
  static String? detectCardBrand(String? number) {
    ...
    if (RegExp(r'^4\d{12}(\d{3})?(\d{3})?$').hasMatch(n)) return 'visa';

    // MasterCard: 51-55 or 2221-2720 (16)
    if (RegExp(
      r'^(5[1-5]\d{14}|2(2[2-9]\d|[3-6]\d{2}|7(0\d|1\d|20))\d{12})$',
    ).hasMatch(n))
      return 'mastercard';

    // Amex: 34 or 37 (15)
    if (RegExp(r'^(34|37)\d{13}$').hasMatch(n)) return 'amex';

    // Discover: various ranges (16/19)
    if (RegExp(
      r'^(6011\d{12}|65\d{14}|64[4-9]\d{13}|622(12[6-9]|1[3-9]\d|[2-8]\d{2}|9([01]\d|2[0-5]))\d{10})$',
    ).hasMatch(n))
      return 'discover';

    // JCB: 3528-3589 (16)
    if (RegExp(r'^(35[2-8]\d{13})$').hasMatch(n)) return 'jcb';

    // Diners Club: 300-305, 36, 38 (14)
    if (RegExp(r'^(3(0[0-5]|[68]\d)\d{11})$').hasMatch(n)) return 'diners';

    // UnionPay: 62... (16-19)
    if (RegExp(r'^(62\d{14,17})$').hasMatch(n)) return 'unionpay';

    return null;
  }

  /// Validate that the card brand is one of [allowedBrands].
  static Validator isCardBrandOneOf(
    Iterable<String> allowedBrands, [
    String? message,
  ]) {
    final set = allowedBrands.map((e) => e.toLowerCase()).toSet();
    return (value) {
      final brand = detectCardBrand(value?.toString());
      return (brand != null && set.contains(brand))
          ? null
          : (message ??
                Messages.t(
                  'unsupported_card_brand',
                  fallback: 'Unsupported card brand',
                ));
    };
  }
  ...
  static Validator pin([String? message]) {
    ...
    return (value) => (value is String && _pin.hasMatch(value))
        ? null
        : (message ?? Messages.t('invalid_pin'));
  }
  ...
  static Validator passwordStrong([String? message]) {
    ...
    return (value) => (value is String && _pwdStrong.hasMatch(value))
        ...
        : (message ??
              Messages.t(
                'password_not_strong',
                fallback: 'Password not strong enough',
              ));
  }

  static Validator passwordWeak([String? message]) {
    // รหัสผ่านพื้นฐาน: ความยาวอย่างน้อย 4 ตัวอักษร
    return (value) => (value is String && value.length >= 4)
        ? null
        : (message ??
              Messages.t('password_too_weak', fallback: 'Password too weak'));
  }

  static Validator passwordNumber([String? message]) {
    // รหัสผ่านเป็นตัวเลขล้วน
    return (value) => (value is String && _pwdNumber.hasMatch(value))
        ? null
        : (message ?? Messages.t('password_number'));
  }

  static Validator passwordNumberText([String? message]) {
    // รหัสผ่านต้องมีตัวอักษรและตัวเลข
    return (value) => (value is String && _pwdNumberText.hasMatch(value))
        ? null
        : (message ?? Messages.t('password_number_text'));
  }

  static Validator passwordNumberOrText([String? message]) {
    // รหัสผ่านเป็นตัวอักษรหรือตัวเลขเท่านั้น
    return (value) => (value is String && _pwdNumberOrText.hasMatch(value))
        ? null
        : (message ?? Messages.t('password_number_or_text'));
  }
  ...
  static Validator passwordNumberTextSpecial([String? message]) {
    ...
    return (value) => (value is String && _pwdNumberTextSpecial.hasMatch(value))
        ? null
        : (message ?? Messages.t('password_number_text_special'));
  }
  ...
  static Validator passwordLettersWithUppercase([String? message]) {
    ...
    return (value) =>
        (value is String && _pwdLettersOnlyWithUpper.hasMatch(value))
        ? null
        : (message ??
              Messages.t(
                'password_letters_upper_required',
                fallback:
                    'Password must contain letters and at least 1 uppercase',
              ));
  }
  ...
  static Validator passwordTextNumberSpecialRequired([String? message]) {
    ...
    return (value) => (value is String && _pwdNumberTextSpecial.hasMatch(value))
        ...
        : (message ??
              Messages.t(
                'password_text_number_special_required',
                fallback:
  ...
  static Validator phone([String? message]) {
    ...
    return (value) => (value is String && _phone.hasMatch(value))
        ? null
        : (message ?? Messages.t('phone'));
  }
  ...
  static Validator username([String? message]) {
    ...
    return (value) => (value is String && _username.hasMatch(value))
        ? null
        : (message ?? Messages.t('username'));
  }
  ...
  static Validator usernameLettersOnly([String? message]) {
    ...
    return (value) => (value is String && _usernameLettersOnly.hasMatch(value))
        ...
        : (message ??
              Messages.t(
                'username_letters_only',
                fallback: 'Username must contain letters only',
              ));
  }
  ...
  static Validator usernameLowercaseOnly([String? message]) {
    ...
    return (value) =>
        (value is String && _usernameLowercaseOnly.hasMatch(value))
        ? null
        : (message ??
              Messages.t(
                'username_lowercase_only',
                fallback: 'Username must be lowercase letters only',
              ));
  }
  ...
  static Validator usernameAlphanumeric([String? message]) {
    ...
    return (value) => (value is String && _usernameAlnumOnly.hasMatch(value))
        ...
        : (message ??
              Messages.t(
                'username_alnum_only',
                fallback: 'Username must be letters or numbers only',
              ));
  }
  ...
  static Validator nationalId([String? message]) {
    ...
    return (value) {
      if (value is! String || !_nationalId13.hasMatch(value))
        return message ??
            Messages.t('national_id', fallback: 'รหัสประชาชนไม่ถูกต้อง');
      int sum = 0;
      for (int i = 0; i < 12; i++) {
  ...
  static Validator creditCard([String? message]) {
    ...
    return (value) {
      ...
      return sum % 10 == 0
          ? null
          : (message ?? Messages.t('invalid_credit_card'));
    };
  }

  static Validator zipCode([String? message]) {
    // ตรวจสอบรหัสไปรษณีย์ 5 หลัก
    return (value) => (value is String && !_zip5.hasMatch(value))
        ? (message ?? Messages.t('invalid_zip_code'))
        : null;
  }
