...
class Validators {
  ...
  static Validator isPrime([String? message]) {
    ...
    return (value) {
      ...
      if (n == null || n < 2) return (message ?? Messages.t('not_prime', fallback: 'Not a prime number'));
      if (n == 2 || n == 3) return null;
      if (n % 2 == 0 || n % 3 == 0) return (message ?? Messages.t('not_prime', fallback: 'Not a prime number'));
      for (int i = 5; i * i <= n; i += 6) {
        if (n % i == 0 || n % (i + 2) == 0) return (message ?? Messages.t('not_prime', fallback: 'Not a prime number'));
      }
      return null;
    };
  }
  ...
  static Validator isEmailInDomains(Iterable<String> allowedDomains, [String? message]) {
    ...
    return (value) {
      if (value is! String || !value.contains('@')) return message ?? Messages.t('email', fallback: 'Invalid email');
  ...
  static Validator isEmpty([String? message]) {
    ...
    return (value) {
      ...
      if (value is String && value.trim().isEmpty) return (message ?? Messages.t('value_empty'));
      if (value is List && value.isEmpty) return (message ?? Messages.t('value_empty'));
      if (value is Map && value.isEmpty) return (message ?? Messages.t('value_empty'));
      return null;
    };
  }
  ...
  static Validator isNotEmpty([String? message]) {
    ...
    return (value) {
      ...
      if (value is String && value.trim().isEmpty) return (message ?? Messages.t('value_not_empty'));
      if (value is List && value.isEmpty) return (message ?? Messages.t('value_not_empty'));
      if (value is Map && value.isEmpty) return (message ?? Messages.t('value_not_empty'));
      return null;
    };
  }
  ...
  static Validator isISBN10([String? message]) {
    return (value) {
      ...
      if (s.length != 10) return message ?? Messages.t('invalid_isbn10', fallback: 'Invalid ISBN-10');
      int sum = 0;
      for (int i = 0; i < 9; i++) {
        final c = s.codeUnitAt(i);
        if (c < 48 || c > 57) return message ?? Messages.t('invalid_isbn10', fallback: 'Invalid ISBN-10');
        final d = c - 48;
        final weight = 10 - i;
        sum += weight * d;
      }
  ...
  static Validator isISBN13([String? message]) {
    return (value) {
      ...
      if (!_ean13.hasMatch(s)) return message ?? Messages.t('invalid_isbn13', fallback: 'Invalid ISBN-13');
      int sum = 0;
      for (int i = 0; i < 12; i++) {
  ...
  static String? detectCardBrand(String? number) {
    ...
    if (RegExp(r'^(5[1-5]\d{14}|2(2[2-9]\d|[3-6]\d{2}|7(0\d|1\d|20))\d{12})$').hasMatch(n)) return 'mastercard';

    // Amex: 34 or 37 (15)
    if (RegExp(r'^(34|37)\d{13}$').hasMatch(n)) return 'amex';

    // Discover: various ranges (16/19)
    if (RegExp(r'^(6011\d{12}|65\d{14}|64[4-9]\d{13}|622(12[6-9]|1[3-9]\d|[2-8]\d{2}|9([01]\d|2[0-5]))\d{10})$').hasMatch(n)) return 'discover';
