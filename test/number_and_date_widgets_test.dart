...
void main() {
  testWidgets('FormProNumberField accepts only numbers by formatter and binds value', (tester) async {
    final form = FormPro.builder().addField('amount', FormFieldConfig(validators: [Validators.required()])).build();
  ...
  testWidgets('FormProDatePickerField writes picked date (YYYY-MM-DD) via injected picker', (tester) async {
    final form = FormPro.builder().addField('birthday', FormFieldConfig(validators: [Validators.required()])).build();

    DateTime? receivedInit;
    final pickedDate = DateTime(2024, 1, 15);

    Future<DateTime?> picker(BuildContext context, DateTime init, DateTime first, DateTime last, Locale? locale) async {
      receivedInit = init;
      return pickedDate;
    }
