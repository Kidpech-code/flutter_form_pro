...
void main() {
  testWidgets('FormProTextField binds value and shows error', (tester) async {
    final form = FormPro.builder().addField('email', FormFieldConfig(validators: [Validators.required(), Validators.email()])).build();
  ...
  testWidgets('FormProKeyboardSubmit triggers submit on Enter', (tester) async {
    ...
    final form = FormPro.builder().addField('name', FormFieldConfig(validators: [Validators.required()], initialValue: 'Bob')).build();
      ...
      MaterialApp(
        home: Scaffold(
          body: FormProWidget(
            ...
            child: const FormProKeyboardSubmit(autofocus: true, child: Text('content')),
          ),
        ),
      ),
    );
